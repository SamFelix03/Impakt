...
try:
    ...
    # LangGraph uses LangChain integration (per official docs)
    LangGraphOpikTracer = None
    try:
        print(f"[OPIK DEBUG] Attempting to import OpikTracer from opik.integrations.langchain...")
        from opik.integrations.langchain import OpikTracer as LangGraphOpikTracer
        print(f"[OPIK] [OK] LangChain OpikTracer available for LangGraph")
        print(f"[OPIK DEBUG] OpikTracer class: {LangGraphOpikTracer}")
        print(f"[OPIK DEBUG] OpikTracer module: {LangGraphOpikTracer.__module__}")
    except ImportError as e:
        LangGraphOpikTracer = None
        print(f"[OPIK] [WARNING] LangChain OpikTracer not available: {e}")
        import traceback
        traceback.print_exc()
    try:
    ...
    TRACK_LANGGRAPH_AVAILABLE = False
...
if OPIK_AVAILABLE:
    try:
        print(f"[OPIK DEBUG] Initializing Opik...")
        print(f"[OPIK DEBUG] OPIK_API_KEY present: {bool(OPIK_API_KEY)}")
        print(f"[OPIK DEBUG] OPIK_WORKSPACE: {OPIK_WORKSPACE}")
        print(f"[OPIK DEBUG] OPIK_PROJECT: {OPIK_PROJECT}")
        
        # Set project name as environment variable for Opik to pick up
        os.environ["OPIK_PROJECT_NAME"] = OPIK_PROJECT
        print(f"[OPIK DEBUG] Set OPIK_PROJECT_NAME env var to: {OPIK_PROJECT}")
        
        if OPIK_API_KEY:
            print(f"[OPIK DEBUG] Configuring Opik with API key and workspace...")
            configure(api_key=OPIK_API_KEY, workspace=OPIK_WORKSPACE)
        else:
            print(f"[OPIK DEBUG] Configuring Opik with workspace only (no API key)...")
            configure(workspace=OPIK_WORKSPACE)
        
        print(f"[OPIK DEBUG] Creating Opik client...")
        opik_client = Opik()
        print(f"[OPIK DEBUG] Opik client created: {type(opik_client)}")
...
class LLMJudgeEvaluator:
    ...
    def _send_feedback_to_opik(self, trace_name: str, evaluation: Dict[str, Any], metric_name: str = "quality_score"):
        ...
        # Import OpikException at function level to avoid UnboundLocalError
        try:
            from opik.exceptions import OpikException
        except ImportError:
            OpikException = Exception  # Fallback if OpikException not available
        
        try:
            from opik.opik_context import update_current_trace, get_current_trace_data
            
            # Get current trace ID for logging (optional)
            try:
                trace_data = get_current_trace_data()
                if trace_data and hasattr(trace_data, 'trace_id'):
                    current_trace_id = trace_data.trace_id
                    print(f"[OPIK DEBUG] Sending feedback scores to trace: {current_trace_id}")
                elif trace_data and isinstance(trace_data, dict) and 'trace_id' in trace_data:
                    current_trace_id = trace_data['trace_id']
                    print(f"[OPIK DEBUG] Sending feedback scores to trace: {current_trace_id}")
            except Exception:
                print(f"[OPIK DEBUG] Could not get current trace ID (not critical)")
            ...
            if overall_score:
                feedback_scores.append({
                    "name": metric_name,
                    "value": float(overall_score) / 10.0,  # Normalize to 0-1 range
                    "reason": evaluation.get('reasoning', f"Overall {metric_name} score")
                })
                print(f"[OPIK DEBUG] Added feedback score: {metric_name} = {float(overall_score) / 10.0}")
            ...
            for key, value in evaluation.items():
                if key not in ['overall_score', 'reasoning', 'suggested_range'] and isinstance(value, (int, float)):
                    feedback_scores.append({
                        ...
                        "value": float(value) / 10.0,  # Normalize to 0-1 range
                        "reason": f"{key.replace('_', ' ').title()} criterion score"
                    })
                    print(f"[OPIK DEBUG] Added feedback score: {metric_name}_{key} = {float(value) / 10.0}")
            
            # Update current trace with feedback scores
            if feedback_scores:
                print(f"[OPIK DEBUG] Calling update_current_trace with {len(feedback_scores)} feedback scores")
                update_current_trace(feedback_scores=feedback_scores)
                print(f"[OPIK] ✅ Sent {len(feedback_scores)} feedback scores to Opik trace")
            else:
                print(f"[OPIK DEBUG] No feedback scores to send")
        except OpikException as e:
            # If there's no trace in context (e.g., called outside LangGraph), log it
            if "no trace in the context" in str(e).lower():
                print(f"[OPIK DEBUG] ⚠️ No trace context available - feedback scores not sent")
                print(f"[OPIK DEBUG] This is normal when called outside LangGraph execution")
            else:
            ...
            if "no trace in the context" not in error_msg:
                ...
                print(f"[OPIK DEBUG] ⚠️ No trace context - feedback scores not sent")
...
{{
    ...
    "reasoning": "Brief explanation"
        ...
        try:
            print(f"[OPIK DEBUG] Calling LLM judge for disaster detection evaluation...")
            ...
            print(f"[OPIK DEBUG] LLM judge response received")
            evaluation = json.loads(response.choices[0].message.content)
            print(f"[OPIK DEBUG] Parsed evaluation: {evaluation}")
            print(f"[OPIK DEBUG] Overall score: {evaluation.get('overall_score', 'NOT FOUND')}")
            ...
            print(f"[OPIK] ❌ ERROR in disaster detection evaluation: {e}")
            import traceback
            traceback.print_exc()
...
{{
    ...
    "reasoning": "Brief explanation"
        ...
        try:
            print(f"[OPIK DEBUG] Calling LLM judge for relief calculation evaluation...")
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[{"role": "user", "content": prompt}],
                temperature=0,
            ...
            print(f"[OPIK] ❌ ERROR in relief calculation evaluation: {e}")
            import traceback
            traceback.print_exc()
            return {"error": str(e), "overall_score": 0}
